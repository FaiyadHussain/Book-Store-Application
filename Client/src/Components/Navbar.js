    ...
    <div className="flex justify-between items-center p-4  sticky top-0 z-50 bg-white shadow-lg border-b relative">
      ...
      <Link to="/" className="flex items-center space-x-2">
        <img
          ...
          alt="Bookify Logo"
        />
        <h1 className="text-3xl font-bold font-serif text-gray-800 hover:text-blue-600 transition-all duration-300">
          Bookify
        </h1>
      </Link>
      ...
      <div className="hidden md:flex items-center space-x-6">
        <div className="flex space-x-6 font-medium">
          {items.map((item, i) => (
            <Link
              to={item.link}
              className="text-lg text-gray-800 hover:text-blue-500 font-semibold transition duration-300 ease-in-out transform hover:scale-105"
              key={i}
            >
              {item.title}
            </Link>
